...
public class ITResult {
  ...
  public void createReportFile(final long duration) {
    ...
    final File reportFile = new File(this.it.getOutputTestDirectory(), filename);
    Writer fw;
    try {
      fw = newWriter(reportFile, Charset.defaultCharset());
    ...
    if (this.nothingToDo) {
      ...
      txt +=
          (isSuccess() ? "SUCCESS" : "FAIL")
              + " of the test "
              + this.it.getTestName()
              + ((isGeneratedData()) ? ": generate expected data" : ": launch test and comparison")
              + ". Duration = "
              + duration;
   ...
   * @return report text
   */
  private String createReportText(final boolean withStackTrace, final String duration) {
  ...
  private String getCurrentFormatedDate() {
    ...
    final DateFormat df = new SimpleDateFormat("yyyy.MM.dd kk:mm:ss", IT.DEFAULT_LOCALE);

    return df.format(new Date());
  }

  /** Collect all exceptions throw when compare output test generated to output test expected. */
  public void checkNeededThrowException() {
    ...
    for (final ITOutputComparisonResult ocr : this.comparisonsResults) {
      if (!ocr.getStatusComparison().isSuccess()) {
        ...
        setException(new KenetreException(msg.toString()));
      }
    }
  }
  ...
  public String createExceptionText(final boolean withStackTrace) {
    ...
    ITCommandResult lastCommandResult = this.commandsResults.get(this.commandsResults.size() - 1);
   ...
   */
  public void addComparisonsResults(final Set<ITOutputComparisonResult> comparisonsResults) {
